...
import { getAudioContext } from './GameScene';

export function UI() {
  const { gameState, playerId, joinGame, localFuel, isOverheated } = useGameStore();
          ...
          {isAlive && (
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center gap-2">
                <Zap size={16} className={!isOverheated ? "text-cyan-400" : "text-red-500"} />
                <div className="w-32 h-3 bg-black/50 rounded-full border border-white/20 overflow-hidden">
                  <div 
                    className={`h-full transition-all duration-75 ${!isOverheated ? 'bg-cyan-400' : 'bg-red-500'}`}
                    style={{ width: `${localFuel}%` }}
                  />
                </div>
              </div>
            </div>
